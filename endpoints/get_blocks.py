...
@app.get("/blocks/{blockId}", response_model=BlockModel, tags=["Cryptix blocks"])
async def get_block(response: Response, blockId: str = Path(regex="[a-f0-9]{64}")):
    """
    Retrieves detailed block data for a specified block hash (blockId) from the Cryptix blockDAG.
    Attempts to fetch the block details from the Cryptixd node. If unavailable, fetches from the database as a fallback.
    """
    ...
    if "block" in resp["getBlockResponse"]:
        # We found the block in cryptixd. Just use it
        requested_block = resp["getBlockResponse"]["block"]
    else:
        if IS_SQL_DB_CONFIGURED:
            # Didn't find the block in cryptixd. Try getting it from the DB
    ...
    # It's possible that the block from cryptixd does not contain transactions
...
@app.get("/blocks", response_model=BlockResponse, tags=["Cryptix blocks"])
async def get_blocks(
    ...
    the Cryptixd node. If unsuccessful, queries the database as a backup, with a response header
...
@app.get(
    "/blocks-from-bluescore", response_model=List[BlockModel], tags=["Cryptix blocks"]
)
async def get_blocks_from_bluescore(
    response: Response, blueScore: int = 43679173, includeTransactions: bool = False
):
    """
    Lists blocks beginning from a specified bluescore within the Cryptix blockDAG.
    """
    response.headers["X-Data-Source"] = "Database"
