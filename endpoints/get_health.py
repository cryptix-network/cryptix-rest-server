...
class CryptixdResponse(BaseModel):
    cryptixdHost: str = ""
...
class HealthResponse(BaseModel):
    cryptixdServers: List[CryptixdResponse]
    currentBlueScoreNode: int = None
    currentDBStatus: DBCheckStatus


@app.get("/info/health", response_model=HealthResponse, tags=["Cryptix network info"])
async def health_state():
    ...
    for i, cryptixd_info in enumerate(cryptixd_client.cryptixds):
        cryptixds.append(
            {
                "isSynced": cryptixd_info.is_synced,
                "isUtxoIndexed": cryptixd_info.is_utxo_indexed,
                "p2pId": hashlib.sha256(cryptixd_info.p2p_id.encode()).hexdigest(),
                "cryptixdHost": f"CRYPTIXD_HOST_{i + 1}",
                "serverVersion": cryptixd_info.server_version,
            }
        )

    return {
        "cryptixdServers": cryptixds,
